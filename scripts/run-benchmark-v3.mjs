 ...
 * 
 * Usage:
 *   node scripts/run-benchmark-v3.mjs
 *   node scripts/run-benchmark-v3.mjs --repeats=30 --mbti=isotonic --ennea=temp-sweep
 */

import { runCompleteBenchmark } from '../src/core/im-core-v3/benchmark/index.js';
...
for (const arg of args) {
  if (arg.startsWith('--repeats=')) {
    options.repeats = parseInt(arg.split('=')[1], 10);
  } else if (arg.startsWith('--mbti=')) {
    options.calibrations.mbti = arg.split('=')[1];
  } else if (arg.startsWith('--ennea=')) {
    const val = arg.split('=')[1];
    if (val === 'temp-sweep') {
      options.calibrations.ennea = { type: 'temp-sweep', grid: [1.3, 1.4, 1.5, 1.6] };
    } else if (val === 'temp') {
      options.calibrations.ennea = { type: 'temp' };
    } else if (val === 'none') {
      options.calibrations.ennea = { type: 'none' };
    }
  }
}
...
try {
  const report = runCompleteBenchmark(options);
  ...
  if (report.confusion && report.confusion.length > 0) {
    console.log('\n🔍 주요 혼동 패턴 (Top 5):');
    report.confusion.slice(0, 5).forEach((c, i) => {
